...
<xsl:stylesheet version='1.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'
        ...
        <xsl:if test="not(@merge:exclude='true')">
          <iati-activity>
            ...
            <xsl:for-each-group select="current-group()/participating-org" group-by="@role">
              ...
              <xsl:for-each-group select="current-group()[@ref='']" group-by="narrative[1]">
                <xsl:apply-templates select="current-group()[1]"/>
              </xsl:for-each-group>
            </xsl:for-each-group>
  ...
  <!-- no values for either attributes, sub elements or content -->
  <xsl:template match="*[not(@*[.!='']) and not(data(.)) and not(*[data(.)!=''])]" />
  <!-- text elements without any narrative element with actual content -->
  <xsl:template match="title                [@*[.!=''] and not(narrative!='')]"/>
  <xsl:template match="description          [@*[.!=''] and not(narrative!='')]"/>
  <xsl:template match="comment              [@*[.!=''] and not(narrative!='')]"/>
  <xsl:template match="condition            [@*[.!=''] and not(narrative!='')]"/>
