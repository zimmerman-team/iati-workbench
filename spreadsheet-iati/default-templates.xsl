...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="record[starts-with($file, 'Projects')]">
    <xsl:if test="starts-with(merge:entry(., 'IATI Activity Identifier'), $reporting-org) and not(merge:boolean(merge:entry(., 'Exclusion applies?')))">
      <iati-activity default-currency="{merge:entry(.,  'Currency')}"
        ...
        <xsl:if test="merge:entry(.,  'Total Budget') != ''">
          <budget status="1" type="1">
            ...
            <value value-date="{merge:date($start-date)}" currency="EUR">{merge:decimal(merge:entry(.,  'Total Budget'))}</value>
          </budget>
        </xsl:if>
        
        <!-- Policy marker may be in the project file -->
        <xsl:if test="merge:entry(.,  'Policy marker') != ''">
          <policy-marker significance="{merge:entry(.,  'Policy significance')}"
  ...
  <xsl:template match="record[starts-with($file, 'Policy')]">
    <xsl:if test="starts-with(merge:entry(.,  'IATI activity identifier')[1], $reporting-org)">
      <iati-activity merge:id="{merge:entry(.,  'IATI activity identifier')}">
        <policy-marker significance="{merge:entry(.,  'Significance')}" code="{merge:entry(., 'Policy marker')}" vocabulary="1"/>
      </iati-activity>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="record[starts-with($file, 'Sectors')]">
    <xsl:if test="starts-with(merge:entry(.,  'IATI activity identifier'), $reporting-org)">
      <iati-activity merge:id="{merge:entry(.,  'IATI activity identifier')}">
        <sector percentage="{merge:decimal(merge:entry(.,  'Budget percentage'))}" code="{merge:entry(., 'Sector code')}" vocabulary="{(merge:entry(., 'Sector vocabulary'))[1]}">
    ...
    <xsl:if test="starts-with(merge:entry(.,  'IATI activity identifier')[1], $reporting-org)">
      <iati-activity merge:id="{merge:entry(.,  'IATI activity identifier')}">
        <transaction ref="{merge:entry(.,  'Reference')}">
          ...
          <value value-date="{(merge:date(merge:entry(.,  'Value date')), merge:date(merge:entry(., 'Date')))[1]}" currency="{merge:entry(., 'Currency')}">{merge:decimal(merge:entry(., 'Amount'))}</value>
          <description>
  ...
  <xsl:template match="record[starts-with($file, 'Participating')]">
    <xsl:if test="starts-with(merge:entry(.,  'IATI activity identifier')[1], $reporting-org)">
      <iati-activity merge:id="{merge:entry(.,  'IATI activity identifier')}">
        <participating-org role="{merge:entry(.,  'Role')}" type="{merge:entry(., 'Type')}" ref="{merge:entry(., 'Organisation identifier')}" activity-id="{merge:entry(., 'Activity identifier')}">
  ...
  <xsl:template match="record[starts-with($file, 'Documents') or ends-with($file, 'Documents')]">
    <xsl:if test="starts-with(merge:entry(.,  'IATI activity identifier'), $reporting-org)">
      <iati-activity merge:id="{merge:entry(.,  'IATI activity identifier')}">
        <document-link format="{merge:entry(.,  'Format')}" url="{merge:entry(., 'Web address')}">
          <title>
